  ...
  return (
    <div className="min-h-screen bg-[#f1f3f7] text-[#0d1117]">
      ...
      <main className="mx-auto max-w-3xl px-6 py-16 md:px-8">
        <Link href="/blog" className="text-sm font-bold text-[#16255c] hover:underline">
          ← Back to Resources
        </Link>

        <div className="mt-6 flex flex-wrap items-center gap-3">
          <span className="bg-[#c8f23c] px-2.5 py-1 text-[11px] font-bold uppercase text-[#101a3d]">{post.audience}</span>
        ...
        <p className="mt-4 text-lg leading-8 text-[#5b6472]">{post.excerpt}</p>
            ...
            <div key={section.heading}>
              ...
              <div className="mt-3 space-y-4">
                {section.paragraphs.map((p, i) => (
                  <p key={i} className="text-sm leading-7 text-[#0d1117]">{p}</p>
                ))}
              </div>
            </div>
          ))}
        ...
        <div className="mt-16 rounded-[16px] bg-white shadow-[0_1px_3px_rgba(13,17,23,0.05),0_8px_24px_rgba(13,17,23,0.06)] ring-1 ring-[#0d1117]/[0.05] p-8 text-center">
          ...
          <p className="mx-auto mt-2 max-w-md text-[#5b6472]">Post a brief or build a creator profile in a few minutes.</p>
          <Link
            href="/signup"
