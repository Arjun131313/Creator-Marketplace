...
const STATUS_STYLE: Record<string, string> = {
  ...
  archived: "bg-[#10141b]/10 text-[#595e66]",
}
  ...
  if (loading) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <p className="text-[#595e66]">Loading Academy…</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      <div className="border-2 border-[#10141b] bg-white p-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="font-display text-2xl font-extrabold text-[#10141b]">Creator Academy</h1>
            <p className="mt-1 text-sm text-[#595e66]">What you&apos;re teaching and what you&apos;ve bought.</p>
          </div>
          <Link
        ...
        <div className="mt-6 flex gap-2">
          <button
            ...
            className={`border-2 px-4 py-1.5 text-xs font-bold transition-colors ${
              tab === "teaching" ? "border-[#10141b] bg-[#10141b] text-[#f5f3ee]" : "border-[#10141b]/20 text-[#595e66]"
            }`}
          >
          ...
          <button
            ...
            className={`border-2 px-4 py-1.5 text-xs font-bold transition-colors ${
              tab === "purchases" ? "border-[#10141b] bg-[#10141b] text-[#f5f3ee]" : "border-[#10141b]/20 text-[#595e66]"
            }`}
          >
        ...
        <div className="space-y-4">
          {taughtLessons.length === 0 ? (
            <div className="border-2 border-dashed border-[#10141b]/20 p-8 text-center text-[#595e66]">
              ...
              <Link href="/creator/academy/new" className="mt-2 block text-sm font-bold text-[#1a54f0] hover:underline">
            ...
            taughtLessons.map((lesson) => (
              <Link
                ...
                className="flex items-center justify-between border-2 border-[#10141b] bg-white p-6 transition-colors hover:bg-[#eae8e1]/40"
              >
                <div>
                  <div className="flex items-center gap-3">
                    <p className="text-lg font-bold text-[#10141b]">{lesson.title}</p>
                    <span className={`px-2.5 py-0.5 text-[10px] font-bold uppercase ${STATUS_STYLE[lesson.status] ?? "bg-[#10141b]/10 text-[#595e66]"}`}>
                      {lesson.status}
                    </span>
                  </div>
                  <p className="mt-1 text-sm text-[#595e66]">£{lesson.price.toLocaleString()} · {lesson.sales} sold</p>
                </div>
                <p className="font-display text-lg font-extrabold text-[#1a54f0]">
        ...
        <div className="space-y-4">
          {purchases.length === 0 ? (
            <div className="border-2 border-dashed border-[#10141b]/20 p-8 text-center text-[#595e66]">
              ...
              <Link href="/academy" className="mt-2 block text-sm font-bold text-[#1a54f0] hover:underline">
                Browse the Academy →
              </Link>
            </div>
          ) : (
            purchases.map((purchase) => (
              ...
              >
                <div>
                  <p className="text-lg font-bold text-[#10141b]">{purchase.lesson_title}</p>
                  <p className="mt-1 text-sm text-[#8b8f96]">
                    Bought {new Date(purchase.created_at).toLocaleDateString("en-GB")}
                  </p>
                </div>
                <span className="text-sm font-bold text-[#1a54f0]">View content →</span>
              </Link>
            ))
          )}
        </div>
